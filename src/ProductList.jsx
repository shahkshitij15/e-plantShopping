...
function ProductList({ onHomeClick }) {
    ...
    // Reset addedToCart for plants that are no longer in the cart
    React.useEffect(() => {
        const cartPlantNames = cartItems.map(item => item.name);
        setAddedToCart(prev => {
            const updated = { ...prev };
            Object.keys(updated).forEach(name => {
                if (!cartPlantNames.includes(name)) {
                    updated[name] = false;
                }
            });
            return updated;
            ...
            {!showCart ? (
                <div className="product-grid">
                    {plantsArray.map((categoryObj) => (
                        <div key={categoryObj.category} className="plantname_heading">
